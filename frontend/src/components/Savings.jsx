...
function Savings(props) {
  ...
  const [ withErr, setWithErr ] = useState(false)
  ...
  function handleVal(vale) {
    console.log(vale, bal)
    setVal(vale)
    if (cat === "withdraw" && parseFloat(vale) > bal) { setWithErr(true) }
    else { setWithErr(false) }
  }
  ...
  return (
    ...
    >
        <input placeholder="Amount" value={val} onChange={(e) => handleVal(e.target.value)} className="mt-3 p-3 w-full bg-white text-slate-800 text-lg rounded-lg cursor-pointer hover:outline-slate-500 focus:outline-slate-500 outline-1 outline outline-slate-300" />
        <span className="text-xl font-semibold mt-4">{withErr ? <span className="text-red-600">Balance not available</span> : <>Updated Balance: <span className="text-blue-600">₹{calculateBalance(bal, val, cat)}</span></>}</span>
