...
function Loan(props) {
  let { value, cat, emi, start_date, payments } = props
  ...
  return (
    ...
    <div className="mt-12 w-full max-w-5xl rounded-lg shadow-lg bg-slate-100 overflow-hidden">
      <div className="grid grid-cols-2 gap-x-12 p-20 pb-0">
        <div className="flex flex-col">
          <span className="text-xl font-semibold">{cat.charAt(0).toUpperCase() + cat.slice(1)} Loan</span>
          <span className="text-4xl font-semibold mt-4">₹{value.toFixed(2)}</span>
          <span className="text-lg font-medium mt-8">Next Payment on {next_date}</span>
          <span className="text-3xl font-semibold mt-1">₹{next_payment.toFixed(2)}</span>
        </div>
    ...
    >
      ...
      <button onClick={_ => setOpenMod(false)} className="mt-8 ml-auto py-3 px-16 bg-blue-500 text-white text-lg rounded-lg cursor-pointer hover:bg-blue-600 flex flex-row gap-x-3 items-center">Pay <FaArrowRight /></button>
    </Modal>
